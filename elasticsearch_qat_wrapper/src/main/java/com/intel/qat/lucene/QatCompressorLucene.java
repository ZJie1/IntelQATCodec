...
public class QatCompressorLucene {

    private static final Logger LOG = LogManager.getLogger(QatCompressorLucene.class);
    private static final int DEFAULT_DIRECT_BUFFER_SIZE = 640 * 1024;

    // HACK - Use this as a global lock in the JNI layer
    @SuppressWarnings({"rawtypes"})
    private static Class clazz = QatCompressorLucene.class;
    private static boolean nativeQatLoaded = false;

    private static final String LIBRARY_NAME = "QatCodecLucene";
    private static final NativeCodeLoader nativeCodeLoader = new NativeCodeLoader(LIBRARY_NAME);

    static {
        if (nativeCodeLoader.isNativeCodeLoaded() &&
                nativeCodeLoader.buildSupportsQat()) {
            try {
                ...
                QatCompressorDecompressorJNI.initIDs(level);
     ...
     * @param forcePinned
     */
    public QatCompressorLucene(int directBufferSize, boolean useNativeAllocateBB,
                               boolean forcePinned, boolean numa) {
        this.directBufferSize = directBufferSize;
        if (useNativeAllocateBB) {
            LOG.info("Creating ByteBuffer's using nativeAllocateBB.");
            try {
                uncompressedDirectBuf = (ByteBuffer) QatCompressorDecompressorJNI.nativeAllocateBB(directBufferSize,
                        numa, forcePinned);
            } catch (Throwable t) {
            ...
            try {
                compressedDirectBuf = (ByteBuffer) QatCompressorDecompressorJNI.nativeAllocateBB(directBufferSize,
                        numa, forcePinned);
            } catch (Throwable t) {
                LOG.error("Failed to create ByteBuffer using nativeAllocateBB"
     ...
     */
    public QatCompressorLucene(int directBufferSize) {
    ...
    public static String getLibraryName(){
        return QatCompressorDecompressorJNI.getLibraryName();
    }
    ...
    public synchronized int compress(byte[] b, int off, int len)
        ...
        // Compress data
        n = QatCompressorDecompressorJNI.compressBytesDirect();
